...
contextBridge.exposeInMainWorld("amerp", {
  ...
  savePreferences: (preferences, options) => invoke("save-preferences", preferences, options),
  onDeepLink: (handler) => {
  ...
  listEmployees: (options) => invoke("list-employees", options),
  loadEmployee: (id) => invoke("load-employee", id),
  saveEmployee: (employee) => invoke("save-employee", employee),
  archiveEmployee: (id) => invoke("archive-employee", id),
  unarchiveEmployee: (id) => invoke("unarchive-employee", id),
  clockInEmployee: (id) => invoke("clock-in-employee", id),
  clockOutEmployee: (id) => invoke("clock-out-employee", id),
  ...
  saveNonconformance: (record, options) => invoke("save-nonconformance", record, options),
  archiveNonconformance: (id) => invoke("archive-nonconformance", id),
  unarchiveNonconformance: (id) => invoke("unarchive-nonconformance", id),
  deleteNonconformance: (id) => invoke("delete-nonconformance", id),
  exportNonconformancePdf: (id, destinationPath) => invoke("export-nonconformance-pdf", id, destinationPath),
  exportNonconformancesCsv: (filters, destinationPath) => invoke("export-nonconformances-csv", filters, destinationPath),
  chooseNonconformanceAttachments: (id) => invoke("choose-nonconformance-attachments", id),
  openNonconformanceAttachment: (id, attachmentId) => invoke("open-nonconformance-attachment", id, attachmentId),
  openNonconformanceAttachmentRevision: (id, attachmentId, revisionIndex) => invoke("open-nonconformance-attachment-revision", id, attachmentId, revisionIndex),
  archiveNonconformanceAttachment: (id, attachmentId) => invoke("archive-nonconformance-attachment", id, attachmentId),
  unarchiveNonconformanceAttachment: (id, attachmentId) => invoke("unarchive-nonconformance-attachment", id, attachmentId),
  reviseNonconformanceAttachment: (id, attachmentId) => invoke("revise-nonconformance-attachment", id, attachmentId),
  deleteNonconformanceAttachment: (id, attachmentId) => invoke("delete-nonconformance-attachment", id, attachmentId),
  generateNextNonconformanceNumber: () => invoke("generate-next-nonconformance-number"),
  listKanbanCards: () => invoke("list-kanban-cards"),
  loadKanbanCard: (id, options) => invoke("load-kanban-card", id, options),
  saveKanbanCard: (card, options) => invoke("save-kanban-card", card, options),
  ...
  undoKanbanImport: (cardIds) => invoke("undo-kanban-import", cardIds),
  ...
  importKanbanUrlsFromCsv: (filePaths) => invoke("import-kanban-urls-from-csv", filePaths),
  importKanbanCardsFromCsv: (filePaths) => invoke("import-kanban-cards-from-csv", filePaths),
  importKanbanFusionToolLibrary: (filePaths) => invoke("import-kanban-fusion-tool-library", filePaths),
  ...
  saveCustomer: (customer, options) => invoke("save-customer", customer, options),
  loadJob: (id, options) => invoke("load-job", id, options),
  saveJob: (job, options) => invoke("save-job", job, options),
  ...
  choosePartDocuments: (jobId, partId, options) => invoke("choose-part-documents", jobId, partId, options),
  ...
  generateNextInspectionReportNumber: () => invoke("generate-next-inspection-report-number"),
  savePartInspection: (jobId, partId, inspection, options) => invoke("save-part-inspection", jobId, partId, inspection, options),
  ...
  exportPartInspectionPdf: (jobId, partId, destinationPath, reportId, options) => invoke("export-part-inspection-pdf", jobId, partId, destinationPath, reportId, options),

  listMaterials: () => invoke("list-materials"),
  loadMaterial: (id, options) => invoke("load-material", id, options),
  saveMaterial: (material, options) => invoke("save-material", material, options),
  ...
  saveInstrument: (payload, options) => invoke("save-instrument", payload, options),
  ...
  saveLibrary: (library, options) => invoke("save-library", library, options),
  deleteLibrary: (name) => invoke("delete-library", name),
  loadTemplates: () => invoke("load-templates"),
  saveTemplate: (template, options) => invoke("save-template", template, options),
  ...
  getLockStatus: (kind, id) => invoke("get-lock-status", kind, id),
  acquireLock: (kind, id, options) => invoke("acquire-lock", kind, id, options),
  renewLock: (kind, id, token) => invoke("renew-lock", kind, id, token),
  releaseLock: (kind, id, token) => invoke("release-lock", kind, id, token),
  takeOverLock: (kind, id, previousToken, options) => invoke("take-over-lock", kind, id, previousToken, options),
